...
final class MarkdownFormatter {
    ...
    public static function result(string $raw,string $response,bool $skill): string {
        ...
        $tags=$data['tags']??null;
        if(!is_array($tags)||!array_is_list($tags)||count($tags)>20)throw new RuntimeException('AI:n skickade ogiltiga taggar.',422);
        foreach($tags as &$tag){if(!is_string($tag)||mb_strlen($tag)>80||!trim($tag))throw new RuntimeException('AI:n skickade ogiltiga taggar.',422);$tag=trim($tag);}unset($tag);
        $meta['tags']=array_values(array_unique($tags));$meta['updated_at']=gmdate('c');
